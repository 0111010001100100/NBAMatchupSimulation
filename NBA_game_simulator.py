...
# Can be overridden in matchup object creation.
...
class Team:
    ...
    def set_team_stats(self):
        ...
        team_stats = team_stats.sort_values(by=['MP'], ascending=False)
        team_stats = team_stats[team_stats['Rk'] <= MAX_PLAYERS_IN_GAME][['Rk', 'Player', 'MP', 'PTS']]
        team_stats['MPRatio'] = team_stats['MP'] / MINUTES_IN_GAME
        sumRatio = team_stats['MPRatio'].sum()
        # Normalization to make sum of al minutes played 240. (48 mins in a game * 5 players on the floor)
        team_stats['MPRatioAdj'] = team_stats['MPRatio'] - ((sumRatio - 5) / float(MAX_PLAYERS_IN_GAME))
...
class Matchup:
    ...
    Args:
        ...
        home_adv (int): A point advantage for the home team. (Default: 3)
    Attributes:
...
class Simulation:
    ...
    def simulate_single_game(self, s=0):
        '''
        ...
        if s != 0:
            random.seed(s)
        home = (random.gauss(self.matchup.home_weighted_pts, self.matchup.home_team.pts_for_std) + random.gauss(self.matchup.away_team.pts_against, self.matchup.away_team.pts_against_std))/2
        away = (random.gauss(self.matchup.away_weighted_pts, self.matchup.away_team.pts_for_std) + random.gauss(self.matchup.home_team.pts_against, self.matchup.home_team.pts_against_std))/2
        if home > away:
            self.home_wins += 1
        else:
            self.away_wins += 1

    def _simulate_single_game(self, weighted_points, pts_std, s):
        '''
        The function called to test the simulate_single_game_function. It tests the random sampling from the gaussian distribution.
        Args:
            weighted_points (float): The team points to test.
            pts_std (float): The team points standard deviation to test.
            s (int): The random seed.
        Returns:
            The number sampled from the distribution.
        '''
        random.seed(s)
        return random.gauss(weighted_points, pts_std)
    ...
    def print_results(self, n):
        ...
        print("{} won {:.3f}% of the games as the home team.".format(self.matchup.home_team.team, self.home_wins / float(n) * 100))
        print("{} won {:.3f}% of the games as the away team.".format(self.matchup.away_team.team, self.away_wins / float(n) * 100))
